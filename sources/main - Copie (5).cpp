...
/* END */
int findMatch(PixelFeature feat_1, PixelFeature *feat_2, int nb_feat_2) {
	double first = 1e100, second = 1e100 ;
	int pos = -1 ;

	for ( int i = 0 ; i < nb_feat_2 ; i++ ) {
		// Take advantage of Laplacian to speed up matching
		/*
		if ( ipts[i].laplace != ip1.laplace )
			continue ;
        */
        if ( sqrt((feat_1.y - feat_2[i].y)*(feat_1.y - feat_2[i].y)) < 20 ) {
            double distSquare = 0.0 ;
            for ( int n = 0 ; n < 9 ; n++ )
                distSquare += (feat_1.histogram[n] - feat_2[i].histogram[n]) * (feat_1.histogram[n] - feat_2[i].histogram[n]) ;

            if( distSquare < first ) {
                second = first ;
                first = distSquare ;
                pos = i ;
            } else if ( distSquare < second ) {
                second = distSquare ;
            }
        }
	}
...
PixelFound findBestMatch(PixelFeature feat_1, Image2D img_in) {
    PixelFeature feat_test[2] ;
    Image2D img_temp ;
    PixelFound pix ;
	double first = 1e100, second = 1e100 ;
	int posx = -1 ;
    ...
    visionImageAlloc(img_temp, img_in.width, img_in.height) ;
    for ( int x = 10 ; x < img_in.width-10 ; x++ ) {
        for ( int y = 10 ; y < img_in.height-10 ; y++ ) {
            visionGetFeature4(img_in, img_temp, x, y, feat_test, 0) ;

            if ( sqrt((feat_1.y - feat_test[0].y)*(feat_1.y - feat_test[0].y)) < 20 ) {
                double distSquare = 0.0 ;
                for ( int n = 0 ; n < 9 ; n++ )
                    distSquare += (feat_1.histogram[n] - feat_test[0].histogram[n]) * (feat_1.histogram[n] - feat_test[0].histogram[n]) ;
...
int main(int argc, char *argv[]) {
    ...
    while ( done < 5 ) { //Loop until escape is pressed
        ...
        if ( argc-2 == 2 ) {
            ...
            if ( show_line == 1 ) {
                ...
                for ( int i = 0 ; i <= nb_feat[0] ; i++ ) {
                    int pos = findMatch(feat[0][i], feat[1], nb_feat[1]);
                    if (pos != -1) {
                        total_point++;

                        sprintf(calib_text,"[x,y,z]=camera_2D_3D(Glob_g, Glob_d, %d, %d, %d, %d)", feat[0][i].x, feat[0][i].y, feat[1][pos].x, feat[1][pos].y) ;
                        SendScilabJob(calib_text) ;
                        SendScilabJob("mfprintf(fakeview,'%f\t%f\t%f\n', x, y, z) ;") ;

                        sdlLigneLight(scr, camWidth*(argc-2), camHeight, feat[0][i].x, feat[0][i].y, feat[1][pos].x+camWidth, feat[1][pos].y, 100) ;
                    }
                }
                ...
                for ( int i = 0 ; i <= nb_feat[0] ; i++ ) {
                    if ( abs(event.motion.x-feat[0][i].x) < 10 && abs(event.motion.y-feat[0][i].y) < 10 ) {
                        printf("\nfeat[0][%d]\n", i) ;
                        visionPrintFeature(feat[0][i]) ;
                        int pos = findMatch(feat[0][i], feat[1], nb_feat[1]);
                        if ( pos != -1 )
                            sdlLigneLight(scr, camWidth*(argc-2), camHeight, feat[0][i].x, feat[0][i].y, feat[1][pos].x+camWidth, feat[1][pos].y, 100) ;

                        PixelFound pixt = findBestMatch(feat[0][i], image[1]) ;
                        sdlLigneDark(scr, camWidth*(argc-2), camHeight, feat[0][i].x, feat[0][i].y, pixt.x+camWidth, pixt.y, 255) ;

                    }
                }
                for ( int i = 0 ; i <= nb_feat[1] ; i++ ) {
                    if ( abs(event.motion.x-camWidth-feat[1][i].x) < 10 && abs(event.motion.y-feat[1][i].y) < 10 ) {
                        printf("\nfeat[1][%d]\n", i) ;
                        visionPrintFeature(feat[1][i]) ;
                        int pos = findMatch(feat[1][i], feat[0], nb_feat[0]);
                        if (pos != -1)
                            sdlLigneLight(scr, camWidth*(argc-2), camHeight, feat[0][pos].x, feat[0][pos].y, feat[1][i].x+camWidth, feat[1][i].y, 100) ;
                    }
                }
            }
        ...
        printf("\ry = %d x = %d min = %d Threshold = %d Nmore = %d  ", threshold_y, threshold_x, min_score, threshold, max_fast) ;
        time_end = get_time() ;
